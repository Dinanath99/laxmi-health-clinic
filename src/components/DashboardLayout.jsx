...
const MENU_ITEMS = [
  ...
  { name: 'Sales & Billing', icon: ShoppingCart, path: '/billing', tags: ['sell', 'invoice', 'create bill', 'sales'] },
  { name: 'Daily Sell & Exp', icon: Activity, path: '/dailylog', tags: ['expense', 'daily log', 'ledger', 'day', 'track'] },
  { name: 'Suppliers Dues', icon: Users, path: '/suppliers', tags: ['supplier', 'due', 'payables', 'vendors', 'purchase'] },
...
export default function DashboardLayout({ children, setAuth }) {
  ...
  const [searchResults, setSearchResults] = React.useState({ modules: [], db: [] });
  ...
  React.useEffect(() => {
    ...
    const modules = MENU_ITEMS.filter(item => 
  ...
  return (
    <div className="flex h-screen print:h-auto print:block print:overflow-visible bg-slate-50/50 text-slate-900 font-sans overflow-hidden selection:bg-blue-100 selection:text-blue-900">
      ...
      <aside className={`print:hidden fixed inset-y-0 left-0 w-64 glass border-r bg-white/95 backdrop-blur-md border-slate-200/60 flex flex-col z-50 shadow-[4px_0_24px_rgba(0,0,0,0.02)] transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-7 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-slate-800 to-slate-600 tracking-tight">LAXMI CLINIC</h2>
              <p className="text-[10px] font-bold text-blue-500 mt-0.5 uppercase tracking-[0.2em]">{user.role} PANEL</p>
            </div>
          </div>
          <button onClick={() => setIsSidebarOpen(false)} className="md:hidden text-slate-400 hover:text-rose-500">
            <X size={24} />
          </button>
        </div>
        ...
        <nav className="flex-1 py-6 overflow-y-auto">
          <ul className="space-y-1.5 px-4 block">
            {MENU_ITEMS.map((item) => (
              <li key={item.name}>
                <Link to={item.path} onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 font-semibold hover:bg-white hover:text-blue-700 hover:shadow-sm hover:border hover:border-slate-100 transition-all border border-transparent mx-2">
        ...
        <div className="p-6 mt-auto">
          ...
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold text-slate-600 bg-white hover:bg-rose-50 hover:text-rose-600 border border-slate-200 rounded-xl transition-all shadow-sm">
      ...
      <main className="flex-1 flex flex-col min-w-0 relative h-full print:h-auto print:overflow-visible bg-[#f8fafc]">
        ...
        <header className="print:hidden sticky top-0 z-30 px-4 md:px-8 py-4 border-b border-slate-200/50 bg-[#f8fafc]/80 backdrop-blur-md flex justify-between items-center gap-4">
          <button onClick={() => setIsSidebarOpen(true)} className="md:hidden text-slate-600 p-2 bg-white rounded-xl shadow-sm border border-slate-200">
            <Menu size={24} />
          </button>
          <div className="relative w-full max-w-md flex-1" ref={searchContainerRef}>
            ...
            {showSearch && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] border border-slate-100 overflow-hidden z-50 py-2 animate-fade-in-down">
                        ...
                        <li key={result.path}>
                          ...
                          >
                            <div className="bg-slate-100 p-2 rounded-lg">
                              <result.icon size={16} className="text-slate-600" />
                            </div>
                            <div>
                              <p className="text-sm font-bold text-slate-700">{result.name}</p>
                              <p className="text-xs font-medium text-slate-400">Navigate to module</p>
                            </div>
                          </button>
                        </li>
                      ))}
                ...
                {searchResults.db.length > 0 && (
                  <>
                    <div className="px-3 pb-1 mb-1 mt-3 border-b border-slate-100 flex items-center justify-between">
                      <p className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">Database Records</p>
                    </div>
                        ...
                        <li key={rec.id}>
                          ...
                          >
                             <div className="flex items-center justify-between w-full">
                                ...
                                <span className={`text-[10px] px-2 py-0.5 rounded font-black tracking-wide ${rec.type === 'Supplier' ? 'bg-orange-100 text-orange-600' : rec.type === 'Medicine' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600'}`}>{rec.type}</span>
                             </div>
                             <p className="text-xs text-slate-500 mt-1 font-medium">{rec.desc}</p>
                          </button>
                        </li>
                      ))}
                    </ul>
                ...
                {!isSearching && searchResults.modules.length === 0 && searchResults.db.length === 0 && (
                  <div className="p-6 text-center text-slate-500">
                    <Search className="mx-auto mb-2 text-slate-300" size={24} />
                    <p className="text-sm font-semibold text-slate-600">No results found for "{searchQuery}"</p>
                    <p className="text-xs mt-1 text-slate-400">Double check spelling or try a different term.</p>
                  </div>
                )}
              </div>
            )}
